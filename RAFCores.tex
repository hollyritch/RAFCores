...
for a reaction $r$ in the set $C_r$ and denote by $\mathcal{K}\coloneqq\{(K_r,r) \ | \ r\in R 
\text{ explicitly catalyzed}\}$ the set of \emph{catalyzations}. The catalysts of a \emph{catalyzation} 
are denoted by $X(C_r)$. The triple $(X',R',\mathcal{K}')$ is called a catalyzed RN. 

A tuple $(X', R')$ is called a subnetwork of $(X,R)$ if $X'\subseteq X, R'\subseteq R$. We 
say a subnetwork $(X',R')$ is well formed if for an $r\in R': \exists x_1,x_2\in X: 
...
The closure of a set $X'$ w.r.t. to a set $R'$ denoted as $ is the unique minimal set $Y$ such that for 
any $r\in R': 

\begin{definition}[RAF]
	Let $(X',R',C')$ be a catalyzed subnetwork with food-set $F'$ and waste molecules $W'$. Then
	$R'$ is called: 
		\begin{itemize} 
			\item{\emph{reflexive autocatalytic} if $\forall r\in R': \emptyset \neq X(C_r)\subseteq X'$}
			\item{\emph{F-generated} if $\forall r\in R': \emptyset \neq X(C_r)\subseteq X'$}
		\end{itemize}
\end{definition}
