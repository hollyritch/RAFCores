...
the reaction $r$. We denote by 
	\begin{equation}
		...
		\end{split}
	\end{equation}
the educts and products of reaction $r$, respectively. A catalyst is an entity $x\in X$ such 
that $s^{-}_{xr}\cdot s^{+}_{xr}\neq 0$. In this case, we say that $r$ is explicitly catalyzed 
by $x$. Note that a reaction may have multiple catalysts. We thereby collect all catalysts 
for a reaction $r$ in the set $\mathcal{K}_r$ and denote by $\mathcal{K}\coloneqq\{(K_r,r) \ | \ r\in R 
\text{ explicitly catalyzed}\}$ the set of \emph{catalyzations}. The catalysts of a \emph{catalyzation} 
are denoted by $X(\mathcal{K}_r)$. The triple $(X',R',\mathcal{K}')$ is called a catalyzed RN. 

A tuple $(X', R')$ is called a subnetwork of $(X,R)$ if $X'\subseteq X, R'\subseteq R$. We 
...
respectively. A triple $(X',R',\mathcal{K}')$ such that $X'\subseteq X, R'\subseteq R, 
\mathcal{K}'\subseteq \mathcal{K}$ is called a catalyzed subnetwork.

The closure of a set $X'$ w.r.t. to a set $R'$ denoted as $c(X'_{R'})$ is the unique minimal set $Y$ such that for 
any $r\in R'$: 
...
\begin{definition}[RAF]
	Let $(X',R',\mathcal{K}')$ be a catalyzed subnetwork with food-set $F'$ and waste molecules $W'$. Then
	$R'$ is called: 
		\begin{itemize} 
			\item{\emph{reflexive autocatalytic} if $\forall r\in R': \emptyset \neq X(\mathcal{K}_r)\subseteq X'$}
			\item{\emph{F-generated} if $\bigcup_{r\in R'} P_r\subseteq c(X'_{R'})$}
		\end{itemize}
\end{definition}
