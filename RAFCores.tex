...
Special attention was put on \emph{irreducible} RAFs, i.e., $\RAFS$ for which no $\RAF'\subset \RAF$ is again a RAF 
...
exhibiting no incoming and no outgoing edges are removed. We start with the following observation.
...
\begin{lemma}
	...
	\RAF \text{s.t. } s^-_{xr}>0 \text{ or } s^+_{xr}>0\}$ is strongly-connected.
\end{lemma}
\begin{proof}
