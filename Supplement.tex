...
Table~\ref{tab:SimulationDetails} specifies different simulation details for different instances of the binary polymer model that 
were used within this contribution. In particular, all models contributed to the generation of Fig.~\ref{M-fig:StrongBlocks}, while 
only \textit{n6\_split} and \textit{n6\_forw} were employed for the generation of Fig.~\ref{M-fig:irrRAFCores} and Fig.~\ref{M-fig:FoodVsNonFood}.
...
\begin{figure}
	...
	\end{minipage}
	\caption{Depiction of the F-reaction composition of 50 arbitrary irreducible 
	RAFs from two instances of the binary polymer model as depicted in 
	Fig.~\ref{M-fig:irrRAFCores} with maximal polymer size of $n=6$. See  
	Table~\ref{tab:} for simulation details.}
