...
\begin{itemize}
  \item {$X = \{0,1\}^{\leq n} \text{ and } \FR \coloneqq \FR \cup \bar{\FR}$} 
  \item {$\FR \coloneqq \{x_i + x_j \rightarrow x_i \cdot x_j : (x_i, x_j \in X) \wedge (|x_i \cdot x_j| \leq n)\}$}
  \item {$\bar{\FR} \coloneqq \{\bar{\fr} \ \vert \ \fr \in \FR \}$} 
  \item {$\mathbb{P}[(x, \fr) \in \cat(\FR)] = p \hspace{100pt} x\in X, \fr \in \FR$}
  \item {$\mathbb{P}[(x, \bar{\fr}) \in \cat(\FR)] = \begin{cases} 1 & \text{if } (x, \fr) \in \cat(\FR) \\ 0 & \text{otherwise} \end{cases} \quad x\in X, \bar{\fr}\in \bar{\FR}$}
  \item {$\food = \{0,1\}^{\leq t}$}
\end{itemize}
